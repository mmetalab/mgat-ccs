...
def feature_generator(df):
    ...
    code_class = df['pred'].values.reshape(-1,1)
...
# Calculate Molecular descriptor   
    ...
    for i in nan_index:
        md_ccs[i[0],i[1]] = 0   
    feats_fp = np.hstack((fp_ccs, code_adduct, code_class))
    feats_md = np.hstack((md_ccs, code_adduct, code_class))
    return code_adduct,code_class,feats_fp,feats_md

def generate_data_loader(df,feats_md,feats_fp,option='train'):
    feats = []
    labels = []
    for index,row in df.iterrows():
        ...
        feats.append([index,smiles,md_adduct_ccs,fp_adduct_ccs,adduct_type,adduct_code])
    if option == 'train':
