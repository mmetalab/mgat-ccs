...
def model_eval_ccs(test_data_loader):
    ...
    with torch.no_grad():
        for i in test_data_loader:
            test_index,test_smiles,test_md, test_fp, test_labels = i[0],i[1],i[2],i[3],i[4] 
            smile = test_smiles
            feats_md = test_md
...
def model_embed_ccs(test_data_loader):
    ...
    with torch.no_grad():
        for i in test_data_loader:
            test_index,test_smiles,test_md, test_fp = i[0],i[1],i[2],i[3]
            smile = test_smiles
            feats_md = test_md
            adj1, g, h = construct_multigraph(smile) 
            h2 = h
    ...
    feats_embed = np.array(feats_embed)
    test_pred = np.array(test_pred)
    return feats_indices,torch.Tensor(feats_embed),torch.Tensor(test_pred)
...
def get_ccs_pair(features,labels):
    ccs_pair = [[features[i][0],features[i][1],torch.from_numpy(features[i][2]).float(),features[i][3],torch.from_numpy(labels[i].reshape(-1,1)).float()] for i in range(len(labels))]      
    adduct_type = [features[i][-2] for i in range(len(labels))]
    adduct_code = [features[i][-1] for i in range(len(labels))]
    class_code = [features[i][2][-1] for i in range(len(labels))]
    # features: [index,smiles,md_adduct_ccs,fp_adduct_ccs,adduct_type,adduct_code]
    # print(features[1][2][-2:],features[1][4],features[1][5])
...
if __name__ == '__main__':
    ...
    print(len(input))
    if mode == 'evaluate' and len(input) == 6:
        labels_file = input[5]
        # test_data_loader_mpnn = torch.load(test_data)
        model = load_checkpoint(model_file)
        # feature_file = '../Data/proc_data/'+datamode+'_feature.pt'
        # labels_file = '../Data/proc_data/'+datamode+'_label.pt'
        features = torch.load(feature_file)
        labels = torch.load(labels_file)
        kf = KFold(n_splits=5)
        sum_df = pd.DataFrame(columns=['dataset','mae','mse','mdae','mare %','mdare %'])
        result_df_details = pd.DataFrame(columns=['model','fold','index','pred','true','adduct_type','adduct_code','class_code'])
        for k, (train, test) in enumerate(kf.split(labels)):
            temp1 = pd.DataFrame(columns=['model','fold','index','pred','true','adduct_type','adduct_code','class_code'])
            temp2 = pd.DataFrame(columns=['model','fold','index','pred','true','adduct_type','adduct_code','class_code'])
            ...
            train_set,train_adduct_type,train_adduct_code,train_class_code = get_ccs_pair(train_features,train_labels)
            test_set,test_adduct_type,test_adduct_code,test_class_code = get_ccs_pair(test_features,test_labels)            
            ...
            # torch.save(test_output, './output/test_output.pth')
        ...
        result_df_details.to_csv('./results/'+datamode+'_result_details_all.csv',index=False)
        result_file = './results/'+'sum_'+datamode+'_result_all.csv'
        ...
        sum_result_file = './results/'+'all_sum_'+datamode+'_result_all.csv'
        'Save summary result file'
