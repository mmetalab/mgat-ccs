...
def model_eval_loss(model,test_data_loader):
    ...
    losses = 0
    with torch.no_grad():
        for test_info, smiles, test_md, test_labels in test_data_loader:
...
def train(args):
    ...
    for i in range(epochs):  
        losses = 0
        losses_val = 0
        for train_info, smiles, train_md, train_labels in train_data_loader:
...
if __name__ == '__main__':
    ...
    if mode == 'train':
        ...
        # features: mol_info,smiles,md_adduct_ccs 
        ccs_pair = [[features[i][0],features[i][1],torch.from_numpy(features[i][2]).float(),torch.from_numpy(labels[i].reshape(-1,1)).float()] for i in range(len(features))]
        ...
        train_data_save = feature_file.replace('.pt','train.pt')
        test_data_save = feature_file.replace('.pt','test.pt')
...
#        [p.join() for p in processes]
        ...
        save_model_file = feature_file.replace('.pt','chkpts.pth')
        torch.save(checkpoint, save_model_file)
        loss_data_save = feature_file.replace('.pt','_loss_data_train.pt')
        torch.save(loss_history,loss_data_save)
        print(len(loss_history))
        loss_data_save_val = feature_file.replace('.pt','_loss_data_val.pt')
